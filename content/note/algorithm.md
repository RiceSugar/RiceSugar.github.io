...
#### [完全背包](https://www.acwing.com/activity/content/problem/content/998/)
```
此处转移方程由
...
- 当`i % prime[j] == 0`时，`prime[j]`等于`i`的最小质因子，`prime[j] * i`的最小质因子为`prime[j]`，因此当`j++`后，`prime[j+1]*i`的最小质因子还是`prime[j]`，而非`prime[j+1]`，不符合算法条件，所以`break`
```cpp
...
- 计算
  - 单hash法
	*其中prime要选择较大的质数(若不是质数，则在哈希的过程中有很大可能集中在某些位置发生冲突*
...
假设n堆石子，石子数目分别是$a_1,a_2,…,a_n$，如果$a_1⊕a_2⊕…⊕a_n≠0$先手必胜；否则先手必败。
